...
/** Analyze a single text string. Returns an NLPBlockAnnotation. */
export async function analyzeText(
  text:     string,
  options?: AnalyzeOptions,
): Promise<NLPBlockAnnotation> {
  const now  = Date.now();
  const lang = "en";
    ...
    if (nerRes.ok && nerRes.data) {
      for (const ent of nerRes.data) {
        ...
        for (let i = 0; i <= tokens.length - entWords.length; i++) {
          const match = entWords.every(
            (w, j) => tokens[i + j]?.text.toLowerCase() === w.toLowerCase(),
          );
          if (match) {
  ...
  // Sentence boundaries — heuristic from punctuation tokens
...
function buildNLPSummary(blocks: STextBlock[]): DocumentNLPSummary {
  ...
  for (const block of blocks) {
    const ann = block.nlp;
    if (!ann) continue;
    if (ann.lang && !lang) lang = ann.lang;
    ...
    for (const tok of ann.tokens) {
      if (tok.isKeyword && tok.keywordScore !== undefined) {
        ...
        if (existing) {
          ...
          kwMap.set(tok.text.toLowerCase(), { score: tok.keywordScore, frequency: 1 });
        }
      }
      if (tok.ner) {
...
/**
 * Analyze all text blocks in a document. Returns a new SDocument
 * with nlp fields populated on each STextBlock, plus a DocumentNLPSummary.
 */
export async function runAnalysis(
  doc:     SDocument,
  options?: AnalyzeOptions,
): Promise<{ doc: SDocument; summary: DocumentNLPSummary; count: number }> {
  let currentBlocks = doc.blocks;
  let count = 0;

  const textBlocks = flattenBlocks(doc.blocks).filter(isTextBlock);
  ...
  for (const block of textBlocks) {
    ...
    try {
      ...
      currentBlocks    = updateBlock(currentBlocks, block.id, (b) =>
        isTextBlock(b) ? { ...b, nlp: annotation } : b,
      );
      count++;
    } catch (e) {
      console.warn(`[runAnalysis] Failed to analyze block ${block.id}:`, e);
    }
  }

  const updatedTextBlocks = flattenBlocks(currentBlocks).filter(isTextBlock);
  const summary           = buildNLPSummary(updatedTextBlocks);

  return { doc: { ...doc, blocks: currentBlocks }, summary, count };
}
