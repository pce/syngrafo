...
/// Window of ~`ctx` chars centred on the first occurrence of `query` in `text`.
/// Falls back to the document beginning when the term is absent.
[[nodiscard]] inline std::string make_context_snippet(std::string_view text,
    ...
    std::transform(ql.begin(),ql.end(),ql.begin(),[](unsigned char c){return (char)std::tolower(c);});
